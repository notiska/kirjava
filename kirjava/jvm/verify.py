...
class Verifier:
    ...
    class Error:
        ...
        def __repr__(self) -> str:
            return "<Verifier.Error(element=%r, message=%r, fatal=%s, data=%s)>" % (
                self.element, self.message, self.fatal, self.data,
            )

        def __str__(self) -> str:
            return "%s (%s): %s" % (self.element, "fatal" if self.fatal else "error", self.message)
