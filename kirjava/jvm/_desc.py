    ...
    if descriptor[0] == "L" and descriptor[-1] == ";":
        ...
        while trailing:
            if not isinstance(element, Array):
                raise ValueError("trailing data %r in reference descriptor" % trailing)
...
def parse_field_descriptor(descriptor: str) -> Type:
    ...
    ------
    ValueError
        If either the descriptor is empty, contains trailing data or contains an invalid type.
    """
    ...
    if remaining:
        ...
        raise ValueError("invalid type %r in descriptor" % type_)
...
def parse_method_descriptor(descriptor: str) -> tuple[tuple[Type, ...], Type]:
    ...
    if preceding:
        raise ValueError("leading data %r in method descriptor" % preceding)
    ...
    if remaining:
        raise ValueError("trailing data %r in method descriptor" % remaining)

    for argument_type in argument_types:
        if argument_type is void_t or type(argument_type) is Invalid:
            raise ValueError("invalid argument type %r in method descriptor" % argument_type)
    if type(return_type) is Invalid:
        raise ValueError("invalid return type %r in method descriptor" % return_type)
