...
class Edge:
    ...
    def __repr__(self) -> str:
        return "<Edge(source=%s, target=%s)>" % (self.source, self.target)
...
class Fallthrough(Edge):
    ...
    def __repr__(self) -> str:
        return "<Fallthrough(source=%s, target=%s, insn=%s)>" % (self.source, self.target, self.insn)

    def __str__(self) -> str:
        if self.insn is not None:
            return "%s : fallthrough %s -> %s" % (self.insn, self.source, self.target)
        return "fallthrough %s -> %s" % (self.source, self.target)
...
class Jump(Edge):
    ...
    def __repr__(self) -> str:
        return "<Jump(source=%s, target=%s, insn=%s)>" % (
            self.source, self.target, self.insn,
        )

    def __str__(self) -> str:
        # if self.fallthrough is not None:
        #     return "%s : [%s] %s -> %s" % (self.insn, self.fallthrough, self.source, self.target)
        return "%s : %s -> %s" % (self.insn, self.source, self.target)
...
class Ret(Jump):
    ...
    def __repr__(self) -> str:
        return "<Ret(source=%s, target=%s, insn=%s)>" % (self.source, self.target, self.insn)
...
class Switch(Edge):
    ...
    def __repr__(self) -> str:
        return "<Switch(source=%s, target=%s, insn=%s, value=%s)>" % (
            self.source, self.target, self.insn, self.value,
        )

    def __str__(self) -> str:
        if self.value is None:
            return "%s [default] %s -> %s" % (self.insn, self.source, self.target)
        return "%s [%i] %s -> %s" % (self.insn, self.value, self.source, self.target)
