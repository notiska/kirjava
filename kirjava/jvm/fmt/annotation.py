...
class ElementValue:
    ...
    def read(cls, stream: IO[bytes], pool: "ConstPool") -> "ElementValue":
        ...
        if subclass is None:  # Not much more we can do at this point.
            raise ValueError("invalid kind %i for element value" % kind)
...
class TargetInfo:
    ...
    def read(cls, stream: IO[bytes]) -> "TargetInfo":
        ...
        if subclass is None:
            raise ValueError("invalid kind 0x%02x for target info" % kind)
...
class TypePath:
    ...
    class Segment:
        ...
        def __repr__(self) -> str:
            return "<TypePath.Segment(kind=%i, index=%i)>" % (self.kind, self.index)

        def __str__(self) -> str:
            return "%i:%i" % (self.kind, self.index)
...
class AnnotationValue(ElementValue):
    ...
    def __repr__(self) -> str:
        return "<AnnotationValue(annotation=%r)>" % self.annotation
...
class ArrayValue(ElementValue):
    ...
    def __repr__(self) -> str:
        return "<ArrayValue(values=%r)>" % self.values
...
class TypeParameterTarget(TargetInfo):
    ...
    def __init__(self, kind: int, index: int) -> None:
        if not kind in TypeParameterTarget.tags:
            raise ValueError("invalid kind 0x%02x for %r" % (kind, type(self)))
...
class TypeParameterBoundTarget(TargetInfo):
    ...
    def __repr__(self) -> str:
        return "<TypeParameterBoundTarget(kind=0x%02x, param_index=%i, bound_index=%i)>" % (
            self.kind, self.param_index, self.bound_index,
        )
...
class LocalVarTarget(TargetInfo):
    ...
    class LocalVar:
        ...
        def __repr__(self) -> str:
            return "<LocalVarTarget.LocalVar(start_pc=%i, length=%i, index=%i)>" % (
