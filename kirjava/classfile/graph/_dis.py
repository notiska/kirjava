...
T = TypeVar("T", bound="Graph")  # To "prove" to numpy that the type in is the same type out, as we can't use Self here.


def disassemble(graph: T, method: MethodInfo, cf: "ClassFile") -> Result[T]:
    """
    ...
    with Result[T].meta(__name__, graph) as result:
