...
from ...model.types import (
    byte_t, char_t, double_t, error_t, float_t, int_t, long_t, null_t, object_t, reference_t, short_t,
...
class ValueCast(Instruction):
    ...
    def trace(self, state: "State") -> Result[list["Step"]]:
        with Result[list["Step"]]() as result:
            entry, steps = state.pop(self, self.type_in).unwrap_into(result)
            # if value.value is not None:  # TODO: Constant value casting.
            #     try:
            #         result = value.value.vcast(self.type_out)
            #         result.debug("(%s)%s", self.type_out, value.value)
            #         return state.step(self, (value,), state.push(result, self))
            #     except ValueError as error:
            #         result.err(error)
            cast = state.push(self, self.type_out).unwrap_into(result)
            steps.append(state.step(self, (entry,), (cast,)))
...
class CheckCast(Instruction):
    ...
    def trace(self, state: "State") -> Result[list["Step"]]:
        with Result[list["Step"]]() as result:
            ref_type: Reference  # mypy
            if not isinstance(self.classref, ClassInfo):
                result.err(TypeError(f"class ref {self.classref!s} is not a class constant"))
                ref_type = reference_t
            else:
                ref_type = self.classref.get_type().unwrap_into(result, reference_t)
            entry, steps = state.pop(self, reference_t).unwrap_into(result)
            cast = state.push(self, entry.cast(ref_type, self)).unwrap_into(result)
            steps.append(state.step(self, (entry,), (cast,)))
            return result.ok(steps)
        return result
...
class InstanceOf(Instruction):
    ...
    def trace(self, state: "State") -> Result[list["Step"]]:
        with Result[list["Step"]]() as result:
            ...
            if entry.type == ref_type or ref_type == object_t:
                result.debug("%s instanceof %s (exact type match)", entry, ref_type)
                # A point on the same classes loaded from different classloaders: this won't fail (I don't think)
                # because if you were to cast an object to a class loaded by a different classloader, you would get a
                # java/lang/ClassCastException.
                instof = state.push(self, Integer(1)).unwrap_into(result)
            elif entry.type == null_t or isinstance(entry.value, Null):
                result.debug("%s instanceof %s (null type/value match)", entry, ref_type)
                instof = state.push(self, Integer(0)).unwrap_into(result)
            else:
                instof = state.push(self, int_t).unwrap_into(result)

            steps.append(state.step(self, (entry,), (instof,)))
            return result.ok(steps)
        return result
