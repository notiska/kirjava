    ...
    def trace(self, context: "Context") -> Tuple[Optional["Frame"], Optional["InsnBlock"]]:
        ...
        if not isinstance(entry.type, ReturnAddress):
            ...
            if not context.do_raise:
                return None, None
            raise NotAReturnAddressError(self, entry.type)
        ...
        if type(entry.type.source) is JsrJumpEdge:
            origin_block = entry.type.source.from_
            jump_edge = entry.type.source
        elif type(entry.type.source) is JsrFallthroughEdge:
            origin_block = entry.type.source.from_
            fallthrough_edge = entry.type.source
        elif type(entry.type.source) is InstructionInBlock:
            origin_block = entry.type.source.block

        if origin_block is None:
            if not context.do_raise:
                return None, None
            raise NotASubroutineError(self, entry.type.source)
